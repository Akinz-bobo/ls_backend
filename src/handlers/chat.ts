...
import { chatMetrics } from '../utils/chat-metrics'

// In-memory storage (should be replaced with Redis in production)
const chatRooms = new Map<string, ChatRoom>()
const userSessions = new Map<string, any>()
const typingTimeouts = new Map<string, NodeJS.Timeout>()
const moderationActions = new Map<string, ModerationAction[]>()
...
function createSuccessResponse(message?: string, data?: any): SuccessResponse {
  return {
    success: true,
    message,
    data,
...
function validateMessageContent(content: string, maxLength: number): { valid: boolean; error?: string } {
...
function checkRateLimit(user: ChatUser, slowModeSeconds: number): { allowed: boolean; waitTime?: number } {
...
export default function chatHandler(io: Server) {
  ...
  io.on('connection', (socket: Socket) => {
    chatMetrics.incrementConnections()
    console.log(`💬 Chat client connected: ${socket.id} (Active: ${chatMetrics.getMetrics().activeConnections})`)
    ...
    socket.on('join-chat', async (broadcastId: string, userInfo: any) => {
      try {
        ...
        if (!chatRooms.has(broadcastId)) {
          ...
          chatMetrics.setRoomCount(chatRooms.size)
        }
        ...
        io.to(`chat-${broadcastId}`).emit('users-updated', {
          users: Array.from(room.users.values()).map((u: ChatUser) => ({
            id: u.id,
            username: u.username,
            role: u.role,
            avatar: u.avatar,
    ...
    socket.on('send-message', async (broadcastId: string, messageData: any) => {
      try {
        ...
        if (!rateLimitCheck.allowed) {
          socket.emit('message-error', createErrorResponse(
            `Slow mode enabled. Please wait ${rateLimitCheck.waitTime} seconds.`,
            'RATE_LIMITED'
          ))
          return
        }
        ...
        chatMetrics.incrementMessages(broadcastId)
    ...
    socket.on('moderate-message', (data: { messageId: string; action: string; broadcastId: string; reason?: string }) => {
      try {
        ...
        switch (action) {
          case 'pin':
            ...
            room.messages.forEach(msg => { if (msg.isPinned) msg.isPinned = false })
    ...
    socket.on('moderate-user', (data: { userId: string; action: string; broadcastId: string; reason?: string; duration?: number }) => {
      try {
        ...
        const moderationAction: ModerationAction = {
          action: action as 'ban' | 'unban' | 'mute' | 'unmute' | 'timeout',
    ...
    socket.on('like-message', (data: { messageId: string; broadcastId: string }) => {
      try {
    ...
    socket.on('broadcast-status-change', (data: { broadcastId: string; isLive: boolean; timestamp: string }) => {
      try {
        ...
        socket.emit('broadcast-status-success', createSuccessResponse(
          `Broadcast status updated to ${isLive ? 'LIVE' : 'OFFLINE'}`
        ))
    ...
    socket.on('disconnect', async (reason) => {
      try {
        chatMetrics.decrementActiveConnections()
        console.log(`💬 Chat client disconnected: ${socket.id} - ${reason} (Active: ${chatMetrics.getMetrics().activeConnections})`)
